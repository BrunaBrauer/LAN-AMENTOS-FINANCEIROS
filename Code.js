// Configuration
const FOLDER_ID = '1BiTb_pB0MiT5kjuhvi1VCgZWlPhpuKRj'; // Replace with your Google Drive folder ID
const SPREADSHEET_ID = '11zJ8mKIHVSdOAHx1yJcq_kSfDVHPY5EyoMdEgNVSHvU'; // Replace with your Google Spreadsheet ID

/**
 * Serves the HTML interface for the web app or returns history data for API calls
 */
function doGet(e) {
  // Check if this is an API call for history
  if (e && e.parameter && e.parameter.action === 'getHistory') {
    const result = getFileHistory();
    return ContentService.createTextOutput(JSON.stringify(result))
      .setMimeType(ContentService.MimeType.JSON);
  }
  
  // Otherwise serve the HTML interface
...
/**
 * Handles POST requests for API calls (saving transactions)
  ...
  try {
    ...
    switch(action) {
      case 'saveTransaction':
        result = processForm(data.formData);
        break;
      case 'getHistory':
        result = getFileHistory();
        break;
      default:
        result = {
          success: false,
